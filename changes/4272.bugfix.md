...
works for every grid kind. `Array.info` no longer raises for sharded arrays
with a rectilinear shard grid, and `Array.chunks` is now defined for any
sharded array (the inner chunks of a shard are always regular).
