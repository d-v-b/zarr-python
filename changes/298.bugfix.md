...
In-flight source reads cannot repopulate entries invalidated by a later mutation, and overlapping writes publish their cached values in source-write order.

Writes to different keys remain concurrent, while delayed writes cannot restore entries removed by concurrent cache invalidation.
