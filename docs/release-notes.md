...
- Fix silent byte-order corruption for structured dtypes with the `bytes` codec: multi-byte fields are now byte-swapped to the codec's configured `endian` on write and decoded honoring it on read, so non-native-endian structured data (e.g. big-endian fields, as produced by virtual references to external data) round-trips correctly.
    ...
    **Behavior change for pre-existing data**: earlier versions did not consistently honor the bytes codec's configured byte order for structured fields. In 3.2.1, decoding used the structured dtype without applying the codec's byte order, while encoding treated the aggregate structured dtype's byte order as native when deciding whether to convert. The corrected reader honors the codec metadata, so results can differ from older readers depending on the stored bytes and field dtypes. The default codec configuration alone does not establish that a mixed-endian structured array is unaffected. ([#4141](https://github.com/zarr-developers/zarr-python/issues/4141))
