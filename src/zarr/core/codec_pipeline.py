...
class Chunk:
    """A chunk with its codec chain and pre-resolved metadata specs.

    Constructed from an iterable of codecs and a chunk ArraySpec.
    Each codec layer transforms the chunk's shape/dtype; these are
    resolved eagerly so that encode/decode can run without
    re-resolving.
    """
    ...
    def shape(self) -> tuple[int, ...]:
        return self.chunk_spec.shape

    @property
    def dtype(self) -> ZDType[TBaseDType, TBaseScalar]:
        return self.chunk_spec.dtype

    def __iter__(self) -> Iterator[Codec]:
        yield from self._aa_codecs
        yield self._ab_codec
        yield from self._bb_codecs

    @property
    def array_array_codecs(self) -> tuple[ArrayArrayCodec, ...]:
        return self._aa_codecs

    @property
    def array_bytes_codec(self) -> ArrayBytesCodec:
        return self._ab_codec

    @property
    def bytes_bytes_codecs(self) -> tuple[BytesBytesCodec, ...]:
        return self._bb_codecs
...
def _write_chunk_compute_default(
    ...
    if existing_bytes is not None:
        existing_array = chunk.decode_chunk(existing_bytes)
    ...
    if chunk_array is None:
        return _DELETED
    chunk_bytes = chunk.encode_chunk(chunk_array)
    if chunk_bytes is None:
        return _DELETED
    return chunk_bytes
...
class BatchedCodecPipeline(CodecPipeline):
    ...
    def __init__(
        ...
        *,
        ...
        if array_bytes_codec is None:
            raise ValueError("array_bytes_codec must be provided.")
        object.__setattr__(self, "array_array_codecs", array_array_codecs)
        object.__setattr__(self, "array_bytes_codec", array_bytes_codec)
        object.__setattr__(self, "bytes_bytes_codecs", bytes_bytes_codecs)
    ...
    def _chunk(self, chunk_spec: ArraySpec) -> Chunk:
        """Build a Chunk with pre-resolved metadata for *chunk_spec*."""
        return Chunk(codecs=tuple(self), chunk_spec=chunk_spec)
    ...
    async def decode(
        ...
        if self._all_sync:
            ...
            chain = self._chunk(first_spec)
            return [chain.decode_chunk(chunk_bytes) for chunk_bytes, _ in items]
    ...
    async def encode(
        ...
        if self._all_sync:
            ...
            _, first_spec = items[0]
            chain = self._chunk(first_spec)
            return [chain.encode_chunk(chunk_array) for chunk_array, _ in items]
    ...
    async def write_batch(
        ...
        if self._all_sync:
            ...
            async def _write_chunk(
                ...
                inner_chain = ab_codec.inner_chunk or chunk
                inner_spec = chunk_spec
                ...
                for coords, chunk_sel, out_sel, _is_complete in prepared.indexer:
                    ...
                    if existing_bytes_inner is not None:
                        existing_array = inner_chain.decode_chunk(existing_bytes_inner)
                    else:
                        existing_array = None
                    ...
                    if not inner_spec.config.write_empty_chunks and merged.all_equal(
                        ...
                        prepared.chunk_dict[coords] = inner_chain.encode_chunk(merged)
    ...
    def read_sync(
        ...
        if n_workers > 0:
            ...
            chunk_arrays: list[NDBuffer | None] = list(
                pool.map(
                    _mark_pool_worker(chunk.decode_chunk),
                    chunk_bytes_list,
                )
            )
    ...
    def write_sync(
        ...
        if n_workers > 0:
            ...
            encoded_list: list[Buffer | None | object] = list(
                pool.map(
                    _mark_pool_worker(_write_chunk_compute_default),
                    existing_bytes_list,
