...
class TestArray(unittest.TestCase):
    ...
    def create_array(
        ...
        **kwargs,
        ...
        init_array_kwargs = {**self.init_array_kwargs, **kwargs}
        init_array(store,
                   shape, 
                   **init_array_kwargs)
        return Array(
    ...
    def test_np_ufuncs(self):
        ...
        zi = self.create_array(
            shape=indices.shape, 
            dtype=indices.dtype, chunks=10, filters=None
        )
        zi[:] = indices
    ...
    def test_object_arrays(self):
        ...
        with pytest.warns(FutureWarning):
            z = self.create_array(shape=10,
            chunks=3, dtype=object, filters=[MsgPack()])
        z.store.close()

        # create an object array using msgpack
        z = self.create_array(shape=10, 
        chunks=3, dtype=object, object_codec=MsgPack())
        z[0] = "foo"
        assert z[0] == "foo"
        z[1] = b"bar"
        assert z[1] == b"bar"
        ...
        z = self.create_array(shape=10, 
        chunks=3, dtype=object, object_codec=Pickle())
        z[0] = "foo"
        assert z[0] == "foo"
        z[1] = b"bar"
        assert z[1] == b"bar"
    ...
    def test_structured_array_contain_object(self):
        ...
        za = self.create_array(
            shape=a.shape, 
            dtype=structured_dtype, object_codec=Pickle()
        )
        za[:] = a
...
class TestArrayWithPath(TestArray):
    ...
    def test_nbytes_stored(self):
        ...
        z = self.create_array(shape=1000, 
        chunks=100)
        expect_nbytes_stored = sum(
...
class TestArrayWithFSStore(TestArray):
    ...
    def create_store(self):
        path = mkdtemp()
        atexit.register(shutil.rmtree, path)
        key_separator = self.init_array_kwargs['dimension_separator']
        store = FSStore(
...
@pytest.mark.skipif(not v3_api_available, reason="V3 is disabled")
class TestArrayWithDBMStoreV3BerkeleyDB(
    TestArrayWithDBMStoreBerkeleyDB, TestArrayWithPathV3
):
    def create_store(self) -> DBMStoreV3:
        bsddb3 = pytest.importorskip("bsddb3")
        path = mktemp(suffix=".dbm")
...
@pytest.mark.skipif(not v3_api_available, reason="V3 is disabled")
class TestArrayWithLMDBStoreV3NoBuffers(
    TestArrayWithLMDBStoreNoBuffers, TestArrayWithPathV3
):
    @staticmethod
    def create_array(array_path="arr1", read_only=False, **kwargs):
        store = self.create_store()
...
@pytest.mark.skipif(have_fsspec is False, reason="needs fsspec")
@pytest.mark.skipif(not v3_api_available, reason="V3 is disabled")
class TestArrayWithFSStoreV3PartialRead(
