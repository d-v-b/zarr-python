...
with ExitStack() as resources:
    ...
    path = Path(directory) / "pyramid.ome.tif"
    ...
    store = resources.enter_context(closing(tifffile.imread(path, return_as="zarr")))
    ...
    with serve_background(store_app(store), host="127.0.0.1") as server:
        ...
        for dataset in datasets:
            ...
            print(
                f"  {dataset['path']}: shape={array.shape} chunks={array.chunks} dtype={array.dtype}"
            )
