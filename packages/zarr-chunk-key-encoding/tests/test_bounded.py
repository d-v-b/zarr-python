...
def test_encode_decode(
    ...
    bounded = encoding.bind_grid_shape(grid_shape)
...
def test_collection_semantics() -> None:
    ...
    assert list(V2ChunkKeyEncoding().bind_grid_shape(())) == ["0"]
    assert len(V2ChunkKeyEncoding().bind_grid_shape(())) == 1
    # A grid with a zero-size dimension holds no chunks at all.
    empty = DefaultChunkKeyEncoding().bind_grid_shape((0, 5))
    assert len(empty) == 0
    assert list(empty) == []
...
def test_from_json_accepts_tuple_and_short_name() -> None:
    ...
    assert bounded == DefaultChunkKeyEncoding().bind_grid_shape((2, 3))
...
def test_bind_equivalent_to_constructor() -> None:
    """`bind_grid_shape` is a convenience for direct construction."""
    encoding = DefaultChunkKeyEncoding()
    assert encoding.bind_grid_shape((2, 3)) == BoundedChunkKeyEncoding(
...
def test_decode_rank_zero_out_of_domain() -> None:
    ...
    bounded = V2ChunkKeyEncoding().bind_grid_shape(())
    with pytest.raises(ChunkKeyOutOfBoundsError):
        bounded.decode("5")
...
def test_invalid_grid_shape() -> None:
    ...
    for grid_shape in ((-1,), (1.5,), ("a",)):
        with pytest.raises(ChunkKeyConfigurationError):
            DefaultChunkKeyEncoding().bind_grid_shape(grid_shape)  # type: ignore[arg-type]
...
def test_contains_without_decode() -> None:
    ...
    with pytest.raises(NotImplementedError):
        "0/0" in _NoDecode().bind_grid_shape((2, 3))  # noqa: B015
    assert "z" in _NoDecode().bind_grid_shape(())
