...
def test_dask_from_array_roundtrip() -> None:
    ...
    lazy = da.from_array(EagerArrayAdapter(source), name=False)
    ...
    blocked = da.from_array(EagerArrayAdapter(source), chunks=(4, 3, 3), name=False)
...
def test_a_masked_source_keeps_its_mask_under_every_partitioning(parts: Any) -> None:
    ...
    got = repartition(LazyArray(data), parts)[:, 1:].result()
    expected = data[:, 1:]
...
def test_dask_can_wrap_opaque_source_without_tokenization() -> None:
    ...
    result = da.from_array(EagerArrayAdapter(view), chunks=(2,), name=False).compute(
