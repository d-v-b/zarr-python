...
def main() -> None:
    ...
    expect_rejected(
        ...
        lambda: ZarrV3ArrayMetadataBuilder().evolve(data_type="uint8", fill_value=300),
    )

    # Known extension names are checked against their canonical configuration.
    expect_rejected(
        "an invalid chunk-key separator",
        lambda: ZarrV3ArrayMetadataBuilder().evolve(
    ...
    array = (
        ZarrV3ArrayMetadataBuilder()
        .evolve(zarr_format=3, node_type="array", shape=(100, 200))
        .evolve(data_type="uint16", fill_value=0)
        .evolve(
