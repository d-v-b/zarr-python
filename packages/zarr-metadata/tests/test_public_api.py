...
def _ruff_dunder_all_sort_key(s: str) -> tuple:  # type: ignore[type-arg]
    """Replicates ruff's RUF022 sort: SCREAMING_SNAKE first, then TitleCase, then dunders.

    Within each group natural sort (numbers sorted numerically) is used.
    """

    def natural_parts(name: str) -> list:  # type: ignore[type-arg]
        parts = re.split(r"(\d+)", name.upper())
        return [int(p) if p.isdigit() else p for p in parts]

    if s.startswith("__") and s.endswith("__"):
        return (2, *natural_parts(s))
    stripped = re.sub(r"[\d_]", "", s)
    if stripped.isupper():
        return (0, *natural_parts(s))
    return (1, *natural_parts(s))
...
def test_all_is_sorted_and_unique() -> None:
    assert zm.__all__ == sorted(zm.__all__, key=_ruff_dunder_all_sort_key)
...
def test_promoted_pairs_drift() -> None:
    pairs = [
        ...
        (zm.INDEX_LOCATION, zm.IndexLocation),
        (zm.DATETIME_UNIT, zm.DateTimeUnit),
        (zm.ROUNDING_MODE, zm.RoundingMode),
        (zm.OUT_OF_RANGE_MODE, zm.OutOfRangeMode),
