...
def test_validator_factory_and_builder_agree(data_type: str, fill_value: int) -> None:
    ...
    try:
        ZarrV3ArrayMetadataBuilder(document).build()
    except MetadataValidationError:
        builder_accepts = False
    else:
        builder_accepts = True

    assert validator_accepts == factory_accepts == builder_accepts
