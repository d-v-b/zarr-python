...
def is_valid_known_codec_name(value: object) -> str | None:
    ...
    The single primitive behind the `TypeGuard` guards in
    ...
    A value carrying an extra member is refused here even though the rules
    layer reports it as a non-blocking `unknown_key`. Failure does not
    exclude TypedDict membership: these checks impose JSON restrictions
    (such as excluding bool from integer fields) beyond Python typing.
    """
