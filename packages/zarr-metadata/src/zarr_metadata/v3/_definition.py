...
def read_field(
    data: JSONValue, kind: type[D], context: Context, loc: Loc = ()
) -> tuple[Resolved[D], Problems]:
    """One refined field whose envelope has been judged, read in `context`.

    What a document's reader calls for a top-level field, whose envelope
    it judged with the document. The definition is asked whenever there
    is one to ask -- a stray member or a malformed `must_understand` says
    nothing about the configuration -- and not when the value names no
    definition or its configuration is not an object, which the envelope
    judgment has said.

    The rules run over a configuration that type-checked and whose nested
    fields' envelopes are sound, as `Definition.judge` asks; whatever a
    nested field's configuration holds, its problems are its own, at its
    own location, and do not keep the rules from running.
    """
    resolved, found = _read(data, as_kind(kind), context, loc)
    return cast("Resolved[D]", resolved), found
